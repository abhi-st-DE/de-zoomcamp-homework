...
2) After setting up the virtual environment and ensuring we are in the same folder as pyproject.toml we need to install some dependencies for this project.
	...
	S-2 : Add sqlalchemy(to interact with database) : uv add sqlalchemy
	S-3 : Add tqdm(to display progress bar while ingesting data) : uv add tqdm
	S-3 : Add click(creating cli interaction) : uv add click
	S-4 : IMP step you need to change your interpretor from your python to the virtual environment python : How to do it ?
